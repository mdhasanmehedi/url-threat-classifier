...
1. Does four-class threat categorization preserve operationally meaningful information that binary detection discards?
2. Are attention weights faithful explanations of model behavior, or should analysts rely on gradient-based attribution instead?

Across three random seeds, PhishFormer, CNN-only, LSTM, and BiLSTM cluster within 0.9 macro-F1 points of one another — no architecture claims formal statistical superiority. A perturbation-based faithfulness audit (the first of its kind in this domain) shows raw attention weights fail the faithfulness test (Wilcoxon p=0.941), while gradient-based attribution — Integrated Gradients and Input×Gradient — passes decisively (p<0.001).
...
**1. Generate deduplication-safe splits** (guarantees no URL leaks across train/val/test):
```bash
cd src
python3 dedup_group_split.py --csv_path ../data/raw/malicious_phish.csv --seed 42
```
...
**5. Inference latency/throughput/memory benchmark:**
```bash
python3 benchmark.py --n_urls 1000 --n_warmup 100 --seed 42
```
