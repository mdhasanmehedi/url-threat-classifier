...
# Pin CPU thread count explicitly so batched-throughput measurements are not
# subject to PyTorch's default thread-count heuristics, which are not
# guaranteed to scale predictably across batch sizes and can behave
# erratically under any concurrent CPU load. Logged for reproducibility
# disclosure (see Data Availability statement).
_N_CPU_THREADS = os.cpu_count() or 4
torch.set_num_threads(_N_CPU_THREADS)
logger.info(f"Pinned torch.set_num_threads({_N_CPU_THREADS})")
...
def benchmark_model(
    ...
    for bs in batch_sizes:
        try:
            ...
            # Warmup (uses n_warmup_batch, independently configurable from
            # the single-URL n_warmup -- previously hardcoded to 20 with no
            # way to change it)
            with torch.no_grad():
            ...
            if "out of memory" in str(e).lower():
                logger.warning(f"    Throughput (batch={bs}): SKIPPED (out of memory)")
                if device.type == "mps":
                    torch.mps.empty_cache()
            else:
                raise
...
def run_all_benchmarks(
    ...
    """Run benchmarks for all models (or one, if only_model is set) on both CPU and MPS."""
    set_seed(seed)
    all_results = {}

    registry = MODEL_REGISTRY if only_model is None else {only_model: MODEL_REGISTRY[only_model]}
    for model_name, (ModelClass, ckpt_filename) in registry.items():
        ...
        cpu_results = benchmark_model(
            ...
            n_urls=n_urls, n_warmup=n_warmup, n_warmup_batch=n_warmup_batch
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_warmup_batch", type=int, default=50)
    parser.add_argument("--seed",     type=int, default=42)
    parser.add_argument("--model", type=str, default=None,
                        choices=list(MODEL_REGISTRY.keys()),
                        help="Benchmark a single model in this process only "
                             "(run once per model from a shell loop for full isolation).")
